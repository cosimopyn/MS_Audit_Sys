...
if [ "$EXIST_DATE"x == "$DATE"x ]; then
  ...
  OUT=`PRIVATE_CONFIG=${QDATA_DIR}/${CON_DD}/tm.ipc geth --exec "loadScript(\"private_exist_contract.js\")" attach ipc:${QDATA_DIR}/${QUO_DD}/geth.ipc`
  echo 'Record stored.'
  echo "Contract address is $ADDRESS"
  echo 'To get record, please use ./run'
else
  sed -i -e "s/var mess.*/var mess=\"$DATE\";/" public_new_contract.js
  OUT=`PRIVATE_CONFIG=${QDATA_DIR}/${CON_DD}/tm.ipc geth --exec "loadScript(\"private_new_contract.js\")" attach ipc:${QDATA_DIR}/${QUO_DD}/geth.ipc`
