...
# Init Quorum node
geth --datadir ${QDATA_DIR}/${QUO_DD} init ../genesis.json

# Start Constellation node
rm -f ${QDATA_DIR}/${CON_DD}/tm.ipc
CMD="constellation-node --url=https://$HOST_IP:$CONSTE_PORT/ --port=$CONSTE_PORT --workdir=${QDATA_DIR}/${CON_DD} --socket=tm.ipc --publickeys=tm.pub --privatekeys=tm.key --othernodes=https://$CLUSTER_IP:$CLUS_CON_PORT/"
...
while $DOWN; do
    sleep 0.1
    DOWN=false
    if [ ! -S "${QDATA_DIR}/${CON_DD}/tm.ipc" ]; then
        DOWN=true
    fi
done
...
# --permissioned
PRIVATE_CONFIG=${QDATA_DIR}/${CON_DD}/tm.ipc nohup geth --datadir ${QDATA_DIR}/${QUO_DD} $ARGS --raftjoinexisting $RAFT_ID --raftport $RAFT_PORT --rpcport $RPC_PORT --port $PORT --unlock 0 --password ../pw.dat 2>>${QDATA_DIR}/logs/quorum.log &
echo "Established Quorum node from host($HOST_IP:$PORT) to Network ($CLUSTER_IP)"
echo '----------------------------------------------------------------------------'
echo "Done"
echo 'To attach to the Geth node, pelase run '"geth attach ${QDATA_DIR}/${QUO_DD}/geth.ipc" 

touch ${QDATA_DIR}/.addresses.dat

ACCOUNT=0x`geth --keystore ${QDATA_DIR}/${QUO_DD}/keystore/ account list | cut -d " " -f 3 | cut -b 2-41` 
CON_PUB_KEY=`cat ${QDATA_DIR}/${CON_DD}/tm.pub`
